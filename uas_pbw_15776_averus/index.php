    ...
    <div class="container">
      ...
      <div id="carouselExample" class="carousel slide">
        <div class="carousel-inner">
          <?php
          // Query ambil data gallery
          $sql = "SELECT * FROM gallery ORDER BY tanggal DESC";
          $hasil = $conn->query($sql);

          // Cek apakah ada data
          if ($hasil->num_rows > 0) {
            $active = true; // Flag untuk item pertama (active)
            while ($row = $hasil->fetch_assoc()) {
              ?>
              <div class="carousel-item <?= ($active) ? 'active' : '' ?>">
                <img src="img/<?= $row["gambar"] ?>" class="d-block w-100" alt="<?= $row["deskripsi"] ?>" />
                <div class="carousel-caption d-none d-md-block">
                  <p><?= $row["deskripsi"] ?></p>
                </div>
              </div>
              <?php
              $active = false; // Ubah jadi false agar item berikutnya tidak active
            }
          } else {
            // Fallback jika tidak ada data gallery
